    ...
    "validating command submission requests" should {
      ...
      "advance ledger time if minLedgerTimeAbs is set" in {
        ...
        commandsValidator.validateCommands(
          ...
          internal.submittedAt,
          internal.maxDeduplicationTime) shouldEqual Right(
          internal.emptyCommands.copy(ledgerEffectiveTime = minLedgerTimeAbs))
      }
      ...
      "advance ledger time if ledgerEffectiveTime is set" in {
        // Note: until ledgerEffectiveTime is removed, it is treated the same as
        // minLedgerTimeAbs
        val minLedgerTimeAbs = internal.submittedAt.plus(internal.timeDelta)
        commandsValidator.validateCommands(
          api.commands.copy(
            ledgerEffectiveTime = Some(TimestampConversion.fromInstant(minLedgerTimeAbs))),
          internal.submittedAt,
          internal.maxDeduplicationTime) shouldEqual Right(
          internal.emptyCommands.copy(ledgerEffectiveTime = minLedgerTimeAbs))
      }
